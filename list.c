...
// the function below add the two linked list and stores the value in list1
void add_lists(Node *list1, Node *list2)
{  
    int len1,len2,i;
    int new_value = 0;
    ...
    // The loop below takes care of the difference in the length of list1 and list2
    // If length of list2 is smaller than list1 then it adds extra node in list2 
    // with the value as addition with zero doesn't change the value.
    
    if (len2 < len1)
    {
        for (i = 1; i < len1; i++)
        {
            insert_at_tail(list2, new_value);
        }
    }
    
    // As we have to add till the end of list1 therefore the below works till 
    // we get a null value from list1
    while (current1 != NULL)
    {
        // The line below is adding the values and replacing it with the current node value
...
// the function below create a duplicate list

// I have created a helper function to insert a new node at the begining of the list
void push(Node** head, int value)
{
    // The below line allocates new node in heap and insert the value
    Node* newNode = (Node*)malloc(sizeof(Node));
    newNode->value = value;
 
    newNode->next = *head;
    
    // Below line returns a pointer to the head of the new list.
...
Node *duplicate_list(Node *head)
{
    Node* current = head;    
    Node* tail;              
    Node temp;  // This node is used to build the duplicate list            
 
    temp.next = NULL;
 
    tail = &temp;                  
    
    // The below loops run till we reach the end of the original list i.e. get a null value
    while (current != NULL)
    {
        // Uses the helper function to add each node at the tail
        push(&(tail->next), current->value);        
        tail = tail->next;     
