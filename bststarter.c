...
// The following set of functions sort the BST into ascending order

// The following code is a helper function which is building the array
// In the function it is sorting the tree using in-order method
// Then it adds the key of the node in the dynamically allocated
// array that we have created in the other function.
void build_array(bstNode *node, int *array, int current_index)
{
    if (node == NULL) return ;
...
// The following function takes the node and create a dynamically 
// allocated array. It takes the total number of nodes and make
// an array of a suitable size using malloc. 
int *array_of_sorted_keys(bstNode *node)
{
   int index = 0;
   int number = num_nodes(node); 
   int *sorted_array = (int *)malloc(sizeof(int)*number);
   build_array(node, sorted_array, index);
   return sorted_array;
}
